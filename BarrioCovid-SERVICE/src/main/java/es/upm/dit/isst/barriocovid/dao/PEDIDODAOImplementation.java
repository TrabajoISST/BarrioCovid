...
public class PEDIDODAOImplementation implements PEDIDODAO{
	private static  PEDIDODAOImplementation instancia = null;
	 ...
	 @Override
		public List<PEDIDO> statusPedidos(int status){
			
			List<PEDIDO> res = new ArrayList<PEDIDO>();
			for (PEDIDO registro : this.readAll())
				if (registro.getStatus() == status) {
					
					res.add(registro);
				}
			
			System.out.println("\n");
			System.out.print("Total de pedidos " + res.size() + " con status igual a:  " + status );
			System.out.println("\n");
